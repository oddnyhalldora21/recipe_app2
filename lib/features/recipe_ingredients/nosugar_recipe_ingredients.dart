import 'package:recipe_app/features/recipe_ingredients/class_recipes.dart';

class SugarFreeRecipes {
  static List<Recipe> sugarFreeRecipes = [
    Recipe(
      id: '61',
      name: 'Sugar-Free Chocolate Chip Cookies',
      imageUrl:
          'https://images.unsplash.com/photo-1499636136210-6f4ee915583e?w=400',
      cookingTime: '25 minutes',
      category: 'sugar-free',
      ingredients: [
        '2 1/4 cups almond flour',
        '1/2 tsp baking soda',
        '1/2 tsp salt',
        '1/2 cup coconut oil, melted',
        '1/2 cup erythritol sweetener',
        '1/4 cup sugar-free maple syrup',
        '2 large eggs',
        '2 tsp vanilla extract',
        '3/4 cup sugar-free chocolate chips',
      ],
      instructions: [
        'Preheat oven to 180°C. Line baking sheets with parchment.',
        'Mix almond flour, baking soda, and salt in a bowl.',
        'In another bowl, whisk melted coconut oil and erythritol.',
        'Add maple syrup, eggs, and vanilla, mix well.',
        'Gradually fold in flour mixture until just combined.',
        'Stir in sugar-free chocolate chips.',
        'Drop rounded tablespoons onto prepared baking sheets.',
        'Bake 10-12 minutes until edges are lightly golden.',
        'Cool on baking sheet for 5 minutes before removing.',
      ],
    ),

    Recipe(
      id: '62',
      name: 'Sugar-Free Brownies',
      imageUrl:
          'https://images.unsplash.com/photo-1606313564200-e75d5e30476c?w=400',
      cookingTime: '35 minutes',
      category: 'sugar-free',
      ingredients: [
        '1/2 cup almond flour',
        '1/4 cup unsweetened cocoa powder',
        '1/2 cup erythritol sweetener',
        '1/4 tsp salt',
        '1/3 cup coconut oil, melted',
        '2 large eggs',
        '1 tsp vanilla extract',
        '1/4 cup sugar-free chocolate chips',
        '1/4 cup chopped walnuts (optional)',
      ],
      instructions: [
        'Preheat oven to 180°C. Line 20cm square pan with parchment.',
        'Mix almond flour, cocoa powder, erythritol, and salt.',
        'In another bowl, whisk melted coconut oil, eggs, and vanilla.',
        'Pour wet ingredients into dry ingredients, mix until smooth.',
        'Fold in chocolate chips and walnuts if using.',
        'Pour batter into prepared pan and smooth top.',
        'Bake 20-25 minutes until toothpick has few moist crumbs.',
        'Cool completely before cutting into squares.',
        'Store covered for up to 5 days.',
      ],
    ),

    Recipe(
      id: '63',
      name: 'Sugar-Free Banana Muffins',
      imageUrl:
          'https://images.unsplash.com/photo-1587668178277-295251f900ce?w=400',
      cookingTime: '30 minutes',
      category: 'sugar-free',
      ingredients: [
        '2 cups almond flour',
        '1 tsp baking soda',
        '1/2 tsp salt',
        '1 tsp cinnamon',
        '3 ripe bananas, mashed',
        '3 large eggs',
        '1/4 cup coconut oil, melted',
        '1/4 cup sugar-free maple syrup',
        '1 tsp vanilla extract',
        '1/2 cup chopped walnuts (optional)',
      ],
      instructions: [
        'Preheat oven to 180°C. Line muffin tin with paper cases.',
        'Mix almond flour, baking soda, salt, and cinnamon.',
        'In another bowl, mash bananas until smooth.',
        'Add eggs, melted coconut oil, maple syrup, and vanilla to bananas.',
        'Pour wet ingredients into dry ingredients, mix until just combined.',
        'Fold in walnuts if using.',
        'Fill muffin cases 3/4 full.',
        'Bake 18-22 minutes until toothpick comes out clean.',
        'Cool in pan for 5 minutes before removing.',
      ],
    ),

    Recipe(
      id: '64',
      name: 'Sugar-Free Chocolate Mousse',
      imageUrl:
          'https://images.unsplash.com/photo-1541599468348-e96984315921?w=400',
      cookingTime: '15 minutes + 2 hours chilling',
      category: 'sugar-free',
      ingredients: [
        '2 ripe avocados',
        '1/4 cup unsweetened cocoa powder',
        '1/4 cup powdered erythritol',
        '1/4 cup coconut milk',
        '2 tsp vanilla extract',
        'Pinch of salt',
        'Fresh berries for garnish',
        'Sugar-free whipped cream (optional)',
      ],
      instructions: [
        'Cut avocados in half, remove pits, and scoop out flesh.',
        'Blend avocados in food processor until completely smooth.',
        'Add cocoa powder, erythritol, coconut milk, vanilla, and salt.',
        'Blend until smooth and creamy, scraping sides as needed.',
        'Taste and adjust sweetness if needed.',
        'Divide among serving glasses.',
        'Chill for at least 2 hours until set.',
        'Garnish with berries and whipped cream before serving.',
        'Best consumed within 24 hours.',
      ],
    ),

    Recipe(
      id: '65',
      name: 'Sugar-Free Lemon Bars',
      imageUrl:
          'https://images.unsplash.com/photo-1565958011703-44f9829ba187?w=400',
      cookingTime: '45 minutes',
      category: 'sugar-free',
      ingredients: [
        'Crust: 1 1/2 cups almond flour',
        '1/4 cup powdered erythritol',
        '1/4 tsp salt',
        '1/4 cup coconut oil, melted',
        'Filling: 4 large eggs',
        '1/2 cup powdered erythritol',
        '1/3 cup fresh lemon juice',
        '2 tbsp lemon zest',
        '2 tbsp almond flour',
      ],
      instructions: [
        'Preheat oven to 160°C. Line 20cm square pan with parchment.',
        'Mix almond flour, erythritol, and salt for crust.',
        'Stir in melted coconut oil until mixture holds together.',
        'Press into prepared pan and bake 15 minutes until golden.',
        'Whisk eggs and erythritol until smooth.',
        'Add lemon juice, zest, and almond flour.',
        'Pour over hot crust and bake 20-25 minutes until set.',
        'Cool completely before cutting into bars.',
        'Dust with additional erythritol if desired.',
      ],
    ),

    Recipe(
      id: '66',
      name: 'Sugar-Free Peanut Butter Cookies',
      imageUrl:
          'https://images.unsplash.com/photo-1590736969955-71cc94901144?w=400',
      cookingTime: '15 minutes',
      category: 'sugar-free',
      ingredients: [
        '1 cup natural peanut butter (no sugar added)',
        '1/2 cup erythritol sweetener',
        '1 large egg',
        '1 tsp vanilla extract',
        '1/2 tsp baking soda',
        '1/4 tsp salt',
      ],
      instructions: [
        'Preheat oven to 180°C. Line baking sheets with parchment.',
        'Mix peanut butter and erythritol until smooth.',
        'Beat in egg and vanilla extract.',
        'Stir in baking soda and salt until combined.',
        'Roll dough into 24 walnut-sized balls.',
        'Place on prepared baking sheets.',
        'Flatten with fork in crisscross pattern.',
        'Bake 10-12 minutes until edges are lightly golden.',
        'Cool on baking sheet for 5 minutes before removing.',
      ],
    ),

    Recipe(
      id: '67',
      name: 'Sugar-Free Chia Pudding',
      imageUrl:
          'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400',
      cookingTime: '10 minutes + 4 hours chilling',
      category: 'sugar-free',
      ingredients: [
        '1/4 cup chia seeds',
        '1 cup unsweetened almond milk',
        '2 tbsp sugar-free maple syrup',
        '1/2 tsp vanilla extract',
        'Pinch of salt',
        'Fresh berries for topping',
        'Chopped nuts for topping',
        'Unsweetened coconut flakes',
      ],
      instructions: [
        'Whisk chia seeds, almond milk, maple syrup, vanilla, and salt.',
        'Let sit for 5 minutes, then whisk again to prevent clumping.',
        'Cover and refrigerate for at least 4 hours or overnight.',
        'Stir well before serving.',
        'Divide between serving glasses.',
        'Top with fresh berries, nuts, and coconut flakes.',
        'Serve chilled.',
        'Keeps in refrigerator for up to 5 days.',
      ],
    ),

    Recipe(
      id: '68',
      name: 'Sugar-Free Apple Crisp',
      imageUrl:
          'https://images.unsplash.com/photo-1568471173238-85490a02cdd8?w=400',
      cookingTime: '40 minutes',
      category: 'sugar-free',
      ingredients: [
        'Filling: 6 large apples, peeled and sliced',
        '1/4 cup erythritol sweetener',
        '2 tbsp almond flour',
        '1 tsp cinnamon',
        '1/4 tsp nutmeg',
        'Topping: 1 cup almond flour',
        '1/2 cup chopped almonds',
        '1/3 cup erythritol sweetener',
        '1/4 cup coconut oil, cold',
        '1/2 tsp cinnamon',
      ],
      instructions: [
        'Preheat oven to 190°C. Grease 23x33cm baking dish.',
        'Toss sliced apples with erythritol, almond flour, cinnamon, and nutmeg.',
        'Place apple mixture in prepared baking dish.',
        'Mix almond flour, chopped almonds, erythritol, and cinnamon.',
        'Cut in cold coconut oil until mixture resembles coarse crumbs.',
        'Sprinkle topping evenly over apples.',
        'Bake 30-35 minutes until topping is golden and apples are tender.',
        'Cool for 10 minutes before serving.',
        'Serve warm if desired.',
      ],
    ),

    Recipe(
      id: '69',
      name: 'Sugar-Free Chocolate Cake',
      imageUrl:
          'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=400',
      cookingTime: '40 minutes',
      category: 'sugar-free',
      ingredients: [
        '1 3/4 cups almond flour',
        '3/4 cup unsweetened cocoa powder',
        '3/4 cup erythritol sweetener',
        '2 tsp baking powder',
        '1 tsp baking soda',
        '1/2 tsp salt',
        '2 large eggs',
        '1 cup unsweetened almond milk',
        '1/2 cup sugar-free applesauce',
        '1/3 cup coconut oil, melted',
        '2 tsp vanilla extract',
      ],
      instructions: [
        'Preheat oven to 180°C. Grease 23cm round cake pan.',
        'Mix almond flour, cocoa powder, erythritol, baking powder, baking soda, and salt.',
        'In another bowl, whisk eggs, almond milk, applesauce, melted coconut oil, and vanilla.',
        'Pour wet ingredients into dry ingredients, mix until smooth.',
        'Pour batter into prepared pan.',
        'Bake 30-35 minutes until toothpick comes out clean.',
        'Cool in pan for 10 minutes.',
        'Turn out onto wire rack to cool completely.',
        'Frost with sugar-free frosting if desired.',
      ],
    ),

    Recipe(
      id: '70',
      name: 'Sugar-Free Energy Bites',
      imageUrl:
          'https://images.unsplash.com/photo-1564355808561-86f5eac0addd?w=400',
      cookingTime: '15 minutes',
      category: 'sugar-free',
      ingredients: [
        '1 cup old-fashioned oats',
        '1/2 cup natural almond butter',
        '1/3 cup sugar-free mini chocolate chips',
        '1/3 cup ground flaxseed',
        '1/4 cup chia seeds',
        '1/4 cup sugar-free maple syrup',
        '1 tsp vanilla extract',
        '1/4 tsp salt',
      ],
      instructions: [
        'Mix all ingredients in a large bowl until well combined.',
        'If mixture is too dry, add 1-2 tbsp water.',
        'If mixture is too wet, add more oats.',
        'Cover and refrigerate for 30 minutes.',
        'Roll mixture into 20 walnut-sized balls.',
        'Store in refrigerator for up to 1 week.',
        'Can be frozen for up to 3 months.',
        'Let thaw for 5 minutes if frozen before eating.',
        'Perfect grab-and-go healthy treat.',
      ],
    ),
  ];

  // Method to get all sugar-free recipes
  static List<Recipe> getAllSugarFreeRecipes() {
    return sugarFreeRecipes;
  }

  // Method to get recipe by id
  static Recipe? getRecipeById(String id) {
    try {
      return sugarFreeRecipes.firstWhere((recipe) => recipe.id == id);
    } catch (e) {
      return null;
    }
  }
}
